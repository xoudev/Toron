...
export async function seedDemoTenant(connectionString: string): Promise<void> {
  ...
  try {
    ...
    // Trois contrôles internes cohérents, mappés sur les objectifs ReCyF
    // réels — la mutualisation multi-référentiels arrive avec ISO 27001 (MVP).
    const controls = [
      [
    ...
    for (const [id, title, description, owner, freq, objectiveRef] of controls) {
      await sql`
      ...
      await sql`
        ...
        FROM requirements r
        WHERE r.framework_id = ${frameworkId} AND r.ref_id = ${objectiveRef}
        ON CONFLICT DO NOTHING`;
    }
  } finally {
    await sql.end();
